...
int a = INT_MAX;
void dfs1(int node, vector<int> adj[], vector<int> &vis, int n, vector<int> &minele)
{
    ...
    for (auto &it : adj[node])
    {
        int temp = it;
        a = min(temp, a);
        dfs1(it, adj, vis, n, minele);
    }
    minele[node].push_back(a);
    a = INT_MAX;
}
int main()
{
    ...
    while (n1--)
    {
        ...
        int m;
        cin >> m;
        vector<int> adj[M + 1]; // 1 based indexing(Assumpsion)
        vi vis(0, M);
        vi minele(INT_MAX, M);
        for (int i = 1; i <= m; i++)
        {
