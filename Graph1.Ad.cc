...
using namespace std;
// ADJACENT C LIST O(N*N) (space complexity)
int main()
{
    int n,m;
    cin>>n>>m;
...
// GRAPH REPRESENTATION O(N+2E) (space complexity)
int main()
{
    int n,m;
    cin>>n>>m;
    vector<int> adj[n+1]; //if no weight
    vector<pair<int,int>> adj[n+1]; // if weight given
    fo(i,0,m)
    {
        int u,v,wt;
        cin>>u>>v>>wt;//add wt if weight given
        adj[u].pb({v,wt}); // Only this if a directed graph
        adj[v].pb({u,wt}); // add this if directed graph
    }
    return 0;
}