...
// Yaad Rakhna
void fun(vector<int> &v, int idx, int n)
{
    if (idx >= n)
    {
        for (auto &i : v)
    ...
    v.pb(idx);
    fun(v, idx + 1, n); // insert kiya aur idx badha aake badho
    v.ppb();
    fun(v, idx + 1, n); // delete kiya aur idx badha aake badho(ayesa lagega ki kuch insert hi nahi kiya)
}
int main()
{
    vector<int> v;
