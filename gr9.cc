...
using namespace std;
void top(int source, vector<int> &vis, vector<pair<int, int>> adj[], stack<int> &s)
{
    vis[source] = 1;
    for (auto &it : adj[source])
    {
        if (!vis[it.first])
            top(it.first, vis, adj, s);
    }
    s.push(source);
}
void dis(stack<int> &st, vector<pair<int, int>> adj[], int src, int n)
{
    int dis[n] ;
    fo(i, 0, n)
        dis[i] = 1e9;
