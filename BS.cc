...
using namespace std;
int mindig(int a[],int start,int end,int n)
{
    while(start<=end){
    int mid=(start+end)/2;
    int next=(mid+1)%n;
...
int binsear(int v[],int start,int end,int m)
{
    while(start<=end)
    {
        int mid=(start+end)/2;
...
int main()
{
    ...
    int start=0,end=n-1;
    int idx=mindig(a,start,end,n);
    if(binsear(a,start,idx-1,m)!=-1)
    cout<<binsear(a,start,idx-1,m);
    else if(binsear(a,idx+1,end,m)!=-1)
    cout<<binsear(a,idx+1,end,m);
    else
