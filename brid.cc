...
using namespace std;
void dfs(int node, int parent, vi &low, vi &tim, vi v[], vi &vis, int time)
{
    vis[node] = 1;
    ...
    for (auto &it : v[node])
    {
        ...
        if (!vis[it])
        {
            dfs(it, node, low, tim, v, vis, time);
            low[node] = min(low[it], low[node]); // compares all the adjacent nodes
            if (low[it] > tim[node])             // formula to check the bridge
                cout << node << " " << it << endl;
        }
        else
