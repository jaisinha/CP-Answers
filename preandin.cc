...
{
    static int idx=0; // will retin it's value and will not disturb the value of idx
    if (start > end)
      return NULL;
    ...
    node->left = buildtree(preorder, inorder, start, pos - 1); // will also check this after the first call of right
    node->right = buildtree(preorder, inorder, pos+1, end); //node->left and node->right makes the tree at backend
    return node;
}
