...
using namespace std;
void bfstra(vector<int> adj[], vector<int> &vis, vector<int> &bfs, int n)
{
    queue<int> q;
    for (int i = 1; i <= n; i++)
    {
        if (!vis[i]) // just to save time
        {
            q.push(i);
            vis[i] = 1; // if this line is not wirtten then multiple digit will occur in the vector
            while (!q.empty())
            {
                int top = q.front();
                q.pop();
